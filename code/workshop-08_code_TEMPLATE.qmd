...
# make all of our gg sets have the same theme 
theme_set(theme_bw())
```
...
drought_exp_clean <- drought_exp |>  # Stating with the drought_exp object 
  ...
  # Makes the species name explicitly a factor, instead of an character , ordering levels 
  mutate(species_name = as_factor(species_name),
         species_name = fct_relevel(species_name,
                                    ...
                                    "Grindelia camporum")) |> 
  # making sure that water_treatment is. a factor and reording the levels 
...
ggplot(data = drought_exp_clean,
  ...
  labs(x = expression(Specific~leaf~area~(cm^2/g)),
       y = "Total mass (g)",
       title = "As SLA inceases, total plant mass increases")
```
...
ggplot(data = drought_exp_clean,
  ...
  labs(x = "Water treatment",
       y = "Total mass (g)",
       title = "Average total mass is highe in well watered treatment \n (and spread is higher in well watered well treatment)") +
  # taking out legend
...
```{r model-fitting}
#have to fit and type these models by hand to see what they say about our response. 
# model 0: null model
model0 <- lm(
  total_g ~ 1, # formula, 1 means no predictors 
